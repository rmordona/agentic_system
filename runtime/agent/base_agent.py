...
#   
################################################################################
# AGENT.md vs SKILL.md — the Hard Boundary
#
# One sentence each (commit this to memory):
#
# AGENT.md
...
#
# Why they appear to overlap:
#   Because agents talk about actions, and skills talk about actions —
#   but from opposite directions.
#
# Dimensions:
#   - Primary axis:        AGENT.md → Judgment | SKILL.md → Execution
#   - Focus:               Decision authority | Operational capability
#   - Question answered:   Should this be done? | How is this done?
#   - Allowed ambiguity:   Some (intent) | None (procedure)
#   - May refuse:          Yes | No (only fails)
#   - Aware of goals:      Yes | No
#   - Aware of ethics:    Yes | No
#   - Reusable across agents: No | Yes
#   - Runtime authority:   No | No (via agent only)
#
# The non-negotiable rule:
#   Skills do not decide.
#   Agents do not explain how.
#
# If you violate this, the system will drift.
################################################################################
#
...
class BaseAgent:
    ...
    Workspace layout:
      workspace/
        ...
        agents/
          <agent_name>/
            agent.json  # or AGENT.md (for sdd)
            context.json
            prompt.md   # does not exist if (non-sdd)
    ...
    def __init__(
        ...
        self.agent_dir = workspace_dir / "agents" / agent_name
        ...
        # --- Load artifacts ---
        self.agent_meta = self._load_json("agent.json")
        self.context_meta = self._load_json("context.json")
        self.prompt_template = self._load_prompt("AGENT.md")
        self.schema = self._load_optional_json("schema.json")

        # --- Skill metadata ---
        self.role = self.agent_meta["role"]
    ...
    def _load_json(self, name: str) -> dict:
        with open(self.agent_dir / name) as f:
            return json.load(f)
