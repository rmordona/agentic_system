...
class StateSchema(BaseModel):
   # ------------------------------------------------------------------
    ...
    session_id: str
    domain: str
    agentContext: Dict[str, Dict[str, Any]] = {}
    # ------------------------------------------------------------------
...
class AgentPlanner:
    ...
    async def __call__(self, state: Union[StateSchema, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        task_obj = Task.model_validate(state.task)

        logger.info(f"State: {state}")

        if task_obj.id == INIT_TASK:
    ...
    ###############################################################################
    def _should_continue(self, state: Union[StateSchema, Dict[str, Any]]) -> str:
    ...
    ################################################################################
    async def compose_initial_plan(self, state: Union[StateSchema, Dict[str, Any]]):
    ...
    ################################################################################
    def validate_route_exit_condition(self, state: Union[StateSchema, Dict[str, Any]]) -> dict:
    ...
    # Routing To Next Open Task
    ################################################################################
    def validate_route_next_open_task(self, state: Union[StateSchema, Dict[str, Any]]) -> dict:
    ...
    ##########################################################################################################
    def switch_agent_context(self, state: Union[StateSchema, Dict[str, Any]]) -> dict:
        """
...
class AgentHITL:
    ...
    async def __call__(self, state: Union[StateSchema, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # This RETURN (not call) tells LangGraph: 
        # "Finish this node, save the state, and wait for external input."
        return Command(
            interrupt={
                "type": "hitl_required",
...
class CoreEngine:
    ...
    async def acquire_new_state(self, user_intent, session_id):
        ...
        init_task = Task(id=INIT_TASK, description="Initial Task", stage="").model_dump()
