...
Exit Condition: `artifact_is_valid(ctx)`  
Next Stages:
- clarification - if `artifact_has_spec_gaps(ctx)`
- ideation — if `artifact_is_valid(ctx)`
...
Exit Condition: `clarifications_resolved(ctx)`  
Next Stages:
- ideation — if `clarifications_resolved(ctx)`
- block — if `clarification_failed(ctx)`
...
Exit Condition: `all_proposals_reviewed(ctx)`  
Next Stages:
- validation — if `accepted_proposals_exist(ctx)`
- clarification — if `critical_issues_detected(ctx)`
- ideation — if `artifact_requires_new_ideas(ctx)`
- block — if `all_proposals_invalid(ctx)`
...
Exit Condition: `artifact_is_valid(ctx)`  
Next Stages:
- approval — if `artifact_is_valid(ctx)`
- spec_revision — if `proposal_conflicts_with_spec(ctx)`
