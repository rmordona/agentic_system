...
class StageGraph:
    ...
    def _add_agent_node(self, agent):
        async def agent_node(state, agent=agent):
            ...
            # Update executed agents per stage
            stage_name = state["stage"]
            executed = state.setdefault("executed_agents_per_stage", {}).setdefault(stage_name, [])
            if agent.role not in executed:
                executed.append(agent.role)
            ...
            return {
                ...
                "executed_agents_per_stage": {stage_name: [agent.role]},
            }
            """
    ...
    def _add_stage_router_node(self):
        ...
        async def stage_router(state: State):
            ...
            print(f"State in stage_router: {state}")
            stage_name = state["stage"]
            current_stage = self.stage_registry.get(stage_name)
            ...
            if self.hitl_callback:
                decision = await self.hitl_callback(state, stage_name)
            ...
            if current_stage.should_exit(state) or not remaining:
                ...
                if next_stage:
                    ...
                    if not next_stage_obj.allowed_agents:
                        ...
                        return {
                            "stage": next_stage, # <-- update state channel
                            "done": True,        # <-- route to first agent in next stage
                        }
        ...
        async def stage_router(state):
            ...
            # If any agents remain, pick the first one
            if remaining:
                next_agent = remaining[0]
                # Must return dict keyed by conditional edges
            ...
            if stage.should_exit(state):
                ...
                # Move to next stage
                next_stage = stage.next_stages[0] if stage.next_stages else None
                if next_stage:
                    next_stage_obj = self.stage_registry.get(next_stage)
                    next_agent = next_stage_obj.allowed_agents[0] if next_stage_obj.allowed_agents else None
                    if next_agent:
                        return {"stage": next_stage, "next_agent": next_agent}
                    else:
                        return {"stage": next_stage}

                return {"done": True}

            # Stay in router if stage not ready to exit
            return {"next_agent": None}

        self.graph.add_node("stage_router", stage_router)

        all_roles = [agent.role for agent in self.agent_registry.all()]

        # Conditional edges: router → agents
